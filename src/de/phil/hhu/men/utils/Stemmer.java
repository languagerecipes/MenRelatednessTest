...
//    }
  ...
  public void add(String wWord) {
      int wLen = wWord.length();
      char[]w =wWord.toCharArray();
        if (i + wLen >= b.length) {
            char[] new_b = new char[i + wLen + INC];
            for (int c = 0; c < i; c++) {
                new_b[c] = b[c];
    ...
    public static void main(String[] args) {
        ...
        s.add("13/3/".toCharArray(), 5);
        s.stem();
System.out.println(s.toString());
        s.add("ment");
        s.stem();
        System.out.println(s.toString()+s.getI_end() + " "+ s.toString().substring(s.getI_end()));
       ;
//      char[] w = new char[501];
//      Stemmer s = new Stemmer();
